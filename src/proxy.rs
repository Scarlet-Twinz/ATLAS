...
impl BackendPool {
    ...
    fn mark_failure(&mut self, address: SocketAddr) {
        if let Some(b) = self.backends.iter_mut().find(|b| b.address == address) {
            b.unhealthy_until = Some(Instant::now() + BACKEND_COOLDOWN);
        }
    }

    fn mark_success(&mut self, address: SocketAddr) {
        if let Some(b) = self.backends.iter_mut().find(|b| b.address == address) {
            b.unhealthy_until = None;
        }
    }
}
...
impl ProxyState {
    ...
    pub async fn health_check(&self, backend: SocketAddr) -> bool {
        match timeout(self.config.connect_timeout, TcpStream::connect(backend)).await {
            Ok(Ok(stream)) => {
                drop(stream);
                self.pool.lock().await.mark_success(backend);
                true
            }
            _ => {
                self.pool.lock().await.mark_failure(backend);
                false
            }
        }
    }
...
impl fmt::Display for ProxyError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidBackend(v) => write!(f, "invalid backend address: {v}"),
            ...
            Self::ResponseBodyTooLarge => f.write_str("response body exceeds maximum size"),
            Self::InvalidContentLength => f.write_str("invalid Content-Length header"),
            Self::InvalidConfiguration(n) => write!(f, "invalid configuration: {n}"),
        }
    }
}
...
pub async fn proxy_connection_with_state(
    ...
    loop {
        ...
        let request = parse_request(&String::from_utf8_lossy(&read_buffer[..header_end]))?;
        ...
        let response = match forward_with_retries(&state, &request, &request_bytes).await {
            ...
            Err(e) => {
                state.metrics.request_failed();
                write_error_response(&mut client, &e).await?;
                return Err(e);
            }
        };
        ...
        state.metrics.request_succeeded(response.len());
        println!(
            "access method={} target={} status={} bytes={}",
            request.method, request.target, status, response.len()
        );
        read_buffer.drain(..total);
...
async fn forward_with_retries(
    ...
    for _ in 0..attempts {
        let backend = { state.pool.lock().await.select() };
...
async fn acquire_upstream(
    ...
    match timeout(state.config.connect_timeout, TcpStream::connect(backend)).await {
        Ok(Ok(s)) => Ok(s),
        Ok(Err(_)) => Err(ProxyError::UpstreamUnavailable),
        Err(_) => Err(ProxyError::UpstreamTimeout),
    }
}

async fn release_upstream(state: &ProxyState, backend: SocketAddr, stream: TcpStream) {
    state
        .connections
        .lock()
        .await
...
async fn forward_once(
    ...
    let mut upstream = acquire_upstream(state, backend).await?;
    upstream.set_nodelay(true)?;
    upstream.write_all(request_bytes).await?;

    match timeout(state.config.request_timeout, read_response(&mut upstream)).await {
        Ok(Ok((response, reusable))) => {
            if reusable {
                release_upstream(state, backend, upstream).await;
            }
            Ok(response)
        }
        Ok(Err(e)) => Err(e),
...
async fn read_response(upstream: &mut TcpStream) -> Result<(Vec<u8>, bool), ProxyError> {
    ...
    let header_end = loop {
        if let Some(e) = find_header_end(&response) {
            break e;
        }
        ...
        if response.len() > MAX_HEADER_BYTES && find_header_end(&response).is_none() {
            return Err(ProxyError::Http(ParseError::HeadersTooLarge));
        }
    };

    let head = parse_response_head(&response[..header_end])?;
    let reusable = response_reusable(&head);

    match head.body {
        ResponseBodyMode::None => response.truncate(header_end),
        ResponseBodyMode::ContentLength(length) => {
            ...
            let total = header_end
                .checked_add(length)
                .ok_or(ProxyError::ResponseBodyTooLarge)?;
        ...
        ResponseBodyMode::Chunked => {
            read_chunked_body(upstream, &mut response, header_end).await?;
        }
        ResponseBodyMode::UntilClose => {
            upstream.read_to_end(&mut response).await?;
            if response.len().saturating_sub(header_end) > MAX_RESPONSE_BODY_BYTES {
                return Err(ProxyError::ResponseBodyTooLarge);
            }
        }
    }
    ...
    Ok((
        response,
        reusable && !matches!(head.body, ResponseBodyMode::UntilClose),
    ))
}
...
async fn read_chunked_body(
    ...
    loop {
        read_buffered_crlf_line(upstream, response, &mut cursor, &mut line).await?;
        let text = std::str::from_utf8(&line[..line.len().saturating_sub(2)])
            .map_err(|_| ProxyError::Http(ParseError::InvalidHeader))?;
        let size_text = text.split(';').next().unwrap_or_default().trim();
        let size = usize::from_str_radix(size_text, 16)
            .map_err(|_| ProxyError::Http(ParseError::InvalidContentLength))?;
        ...
        while response.len() < required {
            let mut chunk = vec![0_u8; BUFFER_SIZE.min(required - response.len())];
            let n = upstream.read(&mut chunk).await?;
            if n == 0 {
                return Err(ProxyError::UpstreamUnavailable);
            }
        ...
        if response[cursor + size..cursor + size + 2] != *b"\r\n" {
...
async fn read_buffered_crlf_line(
    upstream: &mut TcpStream,
    response: &mut Vec<u8>,
    cursor: &mut usize,
    line: &mut Vec<u8>,
    ...
    loop {
        if *cursor + 1 < response.len() {
            if let Some(relative) = response[*cursor..].windows(2).position(|w| w == b"\r\n") {
                let end = *cursor + relative + 2;
                line.extend_from_slice(&response[*cursor..end]);
                *cursor = end;
                return Ok(());
            }
        }
        ...
        if response.len().saturating_sub(*cursor) > MAX_HEADER_BYTES {
            return Err(ProxyError::Http(ParseError::HeadersTooLarge));
        }
    }
}

fn response_reusable(head: &crate::http::HttpResponseHead) -> bool {
    if head.headers.iter().any(|(n, v)| {
    ...
    if head.version == "HTTP/1.0" {
        return head.headers.iter().any(|(n, v)| {
            n.eq_ignore_ascii_case("connection")
                && v.split(',')
...
fn response_status(response: &[u8]) -> Option<u16> {
    ...
    parse_response_head(&response[..end]).ok().map(|h| h.status)
}

fn request_content_length(request: &HttpRequest) -> Result<usize, ProxyError> {
    let Some((_, v)) = request
        .headers
        .iter()
...
async fn write_error_response(
    ...
    let (status, reason) = match error {
        ...
        | ProxyError::ResponseBodyTooLarge => (400, "Bad Request"),
...
fn parse_backends(value: &str) -> Result<Vec<SocketAddr>, ProxyError> {
    value
        ...
        .map(|v| {
            v.parse()
                .map_err(|_| ProxyError::InvalidBackend(v.to_owned()))
        })
        .collect()
}
...
mod tests {
    ...
    fn selects_backends_round_robin() {
        ...
        let mut p = BackendPool::new(&a);
        assert_eq!(p.select(), Some(a[0]));
        assert_eq!(p.select(), Some(a[1]));
        assert_eq!(p.select(), Some(a[0]));
    }

    #[test]
    fn failed_backend_is_temporarily_removed() {
        let a = SocketAddr::from(([127, 0, 0, 1], 9000));
        let mut p = BackendPool::new(&[a]);
        p.mark_failure(a);
    ...
    fn successful_backend_is_restored() {
        ...
        p.mark_failure(a);
        p.mark_success(a);
        assert_eq!(p.select(), Some(a));
    }

    #[test]
    fn parses_backend_list() {
        assert_eq!(
            parse_backends("127.0.0.1:9000,127.0.0.1:9001")
                .unwrap()
                .len(),
            2
        );
    ...
    #[test]
    fn response_reuse_defaults_to_keep_alive() {
        let h = parse_response_head(b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\n").unwrap();
        assert!(response_reusable(&h));
    }

    #[test]
    fn response_reuse_honors_close() {
        let h = parse_response_head(
            b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\n",
        )
        .unwrap();
        assert!(!response_reusable(&h));
...
mod response_runtime_tests {
    ...
    #[tokio::test]
    async fn reads_content_length_response_without_close() {
        ...
        let (r, reuse) = read_response(&mut c).await.unwrap();
        assert!(reuse);
        assert_eq!(r.last().copied(), Some(b'o'));
        server.await.unwrap();
    }
